import { roomUseTargetSchema,layoutObjectTargetSchema,layoutPartTargetSchema,validLayoutHistoryValue,layoutHistoryValue } from './layoutHistory';
import { stairObjectTargetSchema, stairPartTargetSchema, validStairHistoryValue, stairHistoryValue, provisionalStairValue, setStairPart } from './stairHistory';
import { z } from 'zod';
...
const targetSchema = z.discriminatedUnion('kind', [
  stairObjectTargetSchema, stairPartTargetSchema, roomUseTargetSchema, layoutObjectTargetSchema, layoutPartTargetSchema,
...
function valueValid(target: HistoryTarget, value: unknown): boolean {
  switch (target.kind) {
    case 'room-use': case 'layout-object': case 'layout-part': return validLayoutHistoryValue(target,value);
    case 'stair-object': case 'stair-part': return validStairHistoryValue(target,value);
...
export const historyEvidenceSchema = z.object({ version: z.enum(['physical-history-evidence-v1', 'physical-history-evidence-v2', 'physical-history-evidence-v3', 'physical-history-evidence-v4']), events: z.array(z.object({
  ...
  if (evidence.version !== 'physical-history-evidence-v4' && evidence.events.some(event=>event.changes.some(change=>change.target.kind==='room-use'||change.target.kind.startsWith('layout-')))) ctx.addIssue({code:z.ZodIssueCode.custom,message:'Room-use and layout targets require history evidence v4.'});
  if (!['physical-history-evidence-v3','physical-history-evidence-v4'].includes(evidence.version) && evidence.events.some(event=>event.changes.some(change=>change.target.kind.startsWith('stair-')))) ctx.addIssue({code:z.ZodIssueCode.custom,message:'Stair targets require history evidence v3.'});
...
export interface HistoryEvidence { version: 'physical-history-evidence-v1' | 'physical-history-evidence-v2' | 'physical-history-evidence-v3' | 'physical-history-evidence-v4'; events: HistoryEvent[] }
...
export function semanticHistoryValue(target: HistoryTarget, value: any): unknown {
  if ((target.kind === 'room' || target.kind === 'opening' || target.kind === 'level' || target.kind === 'stair-object' || target.kind === 'layout-object') && value !== null && value !== undefined) {
    const { fields: _fields, texts: _texts, index: _index, ...committed } = value; return committed;
  }
  return value;
}
...
export function restoredHistoryValue(target: HistoryTarget, value: unknown, current?: unknown): unknown {
  const next = copyHistory(value) as any;
  if (target.kind === 'stair-part' || target.kind === 'layout-part') return provisionalStairValue(next,current);
...
export function historyValueAt(draft: PhysicalDraft, target: HistoryTarget): unknown {
  if(target.kind==='room-use'||target.kind==='layout-object'||target.kind==='layout-part')return layoutHistoryValue(draft,target);
  if(target.kind==='stair-object'||target.kind==='stair-part')return stairHistoryValue(draft,target);
  ...
  const contract = draft.document.schemaVersion !== 2 ? draft.document.buildingLevels : null;
...
export function validateHistoryEvidence(draft: PhysicalDraft): string | null {
  ...
  for (const event of draft.historyEvidence.events) {
    ...
    if (event.action === 'undo' || event.action === 'redo') {
      ...
      for (const change of event.changes) {
        ...
        if (!original || !equalHistoryValue(semanticHistoryValue(change.target, change.before), semanticHistoryValue(change.target, original.after))
            || !equalHistoryValue(semanticHistoryValue(change.target, change.after), semanticHistoryValue(change.target, restoredHistoryValue(change.target, original.before, original.after)))) return 'A history restoration disagrees with its retained target evidence.';
      }
    ...
    for (const change of event.changes) {
      ...
      if (previous && !['room', 'opening', 'level', 'stair-object', 'layout-object'].includes(change.target.kind) && !equalHistoryValue(previous.after, change.before)) return 'Stored history actions disagree with their preceding target value.';
      if (change.target.kind === 'room' || change.target.kind === 'opening' || change.target.kind === 'level' || change.target.kind === 'stair-object' || change.target.kind === 'layout-object') {
      ...
      if (change.target.kind === 'room' || change.target.kind === 'opening' || change.target.kind === 'level' || change.target.kind === 'stair-object' || change.target.kind === 'layout-object') {
        entities.set(JSON.stringify([change.target.kind === 'stair-object' ? 'stair-object:'+change.target.object : change.target.kind === 'layout-object' ? 'layout-object:'+change.target.object : change.target.kind, change.target.id]), { target: change.target, value: copyHistory(semanticHistoryValue(change.target, change.after)) });
      } else if (change.target.kind === 'layout-part') {
        const retained=entities.get(JSON.stringify(['layout-object:'+change.target.object,change.target.id]));
        if(retained?.value)retained.value.entity[change.target.part]=copyHistory(change.after);
      } else if (change.target.kind === 'stair-part') {
        const retained=entities.get(JSON.stringify(['stair-object:'+change.target.object,change.target.id]));
        if(retained?.value)setStairPart(retained.value.entity,change.target.part,copyHistory(change.after));
