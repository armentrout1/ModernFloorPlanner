import { readDatabaseConfiguration, type DatabaseConfiguration } from './databaseConfiguration';
...
export interface AuthConfiguration {
  ...
  origin: string; callbackUri: string; sessionSecret: string; databaseUrl: string; database: DatabaseConfiguration;
}
...
export function readAuthConfiguration(env: NodeJS.ProcessEnv): AuthConfiguration | null {
  try {
    ...
    const issuerUrl = new URL(issuer); const originUrl = new URL(origin);
    const database = readDatabaseConfiguration(databaseUrl, env);
    ...
    return { issuer, origin, callbackUri, clientId, clientSecret, sessionSecret, databaseUrl, database,
