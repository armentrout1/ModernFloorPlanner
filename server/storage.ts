...
import { z } from 'zod';
import { applicationPrincipals, workspaces, workspaceMemberships, floorPlans,
...
import { withWorkspaceAuthorization, type WorkspaceTransaction as Transaction } from './workspaceAuthorization';
...
export class DatabaseStorage implements AuthorizationStorage {
  ...
  private authorized<T>(identity: VerifiedIdentity, workspaceId: string, permission: 'read' | 'write' | 'owner', operation: (tx: Transaction, principalId: string) => Promise<T>, exclusiveWorkspace = false): Promise<T> {
    return withWorkspaceAuthorization(this.database, identity, workspaceId, permission, operation, exclusiveWorkspace);
  }
