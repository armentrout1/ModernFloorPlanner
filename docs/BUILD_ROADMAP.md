...
Version: 1.8 — M3B integrated acceptance and local closeout, 2026-09-08
Repository: `armentrout1/ModernFloorPlanner`; delivery branch: `main`.
Original audit baseline: `876968e78d7070775e7924f33a3164ba20905d42`; current application/test commit: `c0f0b832537d2e7ac5a3c24786c1b6fd6d99a907`.
Status: **M3 in progress. M3B Slices 1–4 COMPLETE / PRODUCER_VERIFIED locally; NOT DEPLOYED.** Single next bounded task: **M3C — revert one unapplied physical-draft input to its retained committed state; NEXT ELIGIBLE, NOT STARTED.** Remaining M3C, proposed M3D and M4–M8 remain unstarted. Issue #2 remains unreleased; hosting #4 remains PROPOSED.
...
**Fresh final sequence:** npm ci; npm test **350/350**; npm run check; npm run build; npx playwright test --reporter=line **125/125**, zero retries/skips; Git whitespace checks — all pass. The fixture independently gives 120/120 floor/ceiling, 352 gross walls, 21 + 12 deductions, 319 net walls, 41/41/44 ft trim, 17/14 ft casing and one door/window. Explicit floor 10% gives 132 sq ft; top-only gives 96 − 33 = 63 sq ft and 9 ft baseboard. Cleared window height gives unavailable full net walls and an explicitly partial/provisional 256 sq ft subtotal; restoration recovers the numeric basis. Historical snapshot and real browser/Node parity coverage remain included.

Safe new-tab review: **http://127.0.0.1:5179/physical-draft**. A separate origin was needed because the necessary UI repairs could not safely hot reload the protected 5178 session; its watcher was stopped only after testing and before integration. Old 5173/5176/5177/5178 remain stopped; owner tabs were untouched and their browser memory is not claimed saved/transferred. Current canonical source and review-server identity match the delivered tree. **NOT DEPLOYED**; production/database/auth/partner smoke remains unverified.
...
**One next bounded task: M3C — Revert one unapplied physical-draft input. NEXT ELIGIBLE, NOT STARTED.** Add a per-field action returning pending room/opening/offset/waste text to its retained committed value, without changing physical data, evidence, confirmation, unrelated inputs or scope; guard stale/IME/dialog actions and preserve recovery. Current invalid text remains safely recoverable but requires manual retyping, so this is a convenience gap, not a deferred M3B calculation/data-loss defect. Do not start comprehensive undo/redo, room/group movement or redo completed issue #8 interface work.
...
**Single next bounded task: M3C — Revert one unapplied physical-draft input; NEXT ELIGIBLE, NOT STARTED.** Restore only a pending field's raw display from its retained committed value, with stale/IME/dialog guards and no geometry/evidence/confirmation/scope changes. Retain all completed issue #8 work. Comprehensive undo/redo, room/group movement, levels/stairs, hosting, database and later milestones are not activated.
...
Current state: M1, M2A/B/C, M3A and **M3B Slices 1–4 are COMPLETE / PRODUCER_VERIFIED locally; NOT DEPLOYED**. Application/test commit: `c0f0b832537d2e7ac5a3c24786c1b6fd6d99a907`. Issue #10 retains local completion and pending release separately; #2 stays unreleased, #4 PROPOSED and CRM #11 separate/unmerged. **One next bounded task: M3C — Revert one unapplied physical-draft input**, NEXT ELIGIBLE, NOT STARTED. [Current results](MFP_M3B_RESULTS.md) bind fresh **350/350 unit** and **125/125 browser** checks to delivered source and local review identity. Prior evidence remains historical. Issue #9 boundaries in A8/proposed M3D/M7 remain unchanged; levels, stairs and detailed kitchen/bath/trades remain unstarted.
