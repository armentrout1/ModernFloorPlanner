...
Version: 1.17 — M4A local OIDC/session and account checkpoint, 2026-09-09
Repository: `armentrout1/ModernFloorPlanner`; delivery branch: `main`.
Original audit baseline: 876968e78d7070775e7924f33a3164ba20905d42; current application/test commit: c79e458a634131f9e25ed0e4b0bb7497fc07d7ae.
Status: **M4A #20 local OIDC/session and account implementation: COMPLETE / PRODUCER_VERIFIED locally; NOT DEPLOYED.** The completed #19 authorization foundation and M1–M3/initial M3D results remain preserved. M4A's live-connection gate stays open. **Single next task: select and validate the real OIDC provider/client registration, trusted HTTPS application origin and permitted production-database binding under separate authorization.** M4B full physical persistence and M4C exports are separate and NOT STARTED here. Broader #9 stays open, #10 remains release tracking, #2 unreleased, #4 PROPOSED and CRM separate.

Evidence and dated external research: [research and audit](RESEARCH_AND_AUDIT_2026-09-06.md).

## 0. Current M4A local-session and account checkpoint — 2026-09-09
...
At this preserved M3D checkpoint, M4A was next. Its authorization foundation is complete; the current #20 checkpoint above tracks the local OIDC/session and account-facing implementation. The remaining next gate is separately authorized live provider/registration/origin/database validation. Preserve schema 5 and historical source/snapshot compatibility in the persistence contract. Future object families, catalogs, detailed kitchen/bath tools, overview/ghost views, trade systems and M7 recipes are not automatic prerequisites. The user pilot speed/adoption target remains unmeasured. #10 tracks release, #2 remains unreleased and hosting #4 remains PROPOSED.
...
**M4A #19 remains complete; the current #20 checkpoint tracks local OIDC/session and account/workspace implementation and its final verification.** The single following task is separately authorized live provider/client-registration, trusted HTTPS origin and production-database binding validation. No live identity or deployment/database binding is claimed. Current schema 5, full source lineage and v1–v5 capture compatibility must be explicitly covered when secure persistence/export interfaces are selected. Never save a partial legacy representation as though it preserved the current building document.

### M4 — Make standalone use safe and dependable

**M4A #19 foundation remains COMPLETE locally. #20 local OIDC/session and account implementation: COMPLETE / PRODUCER_VERIFIED locally; NOT DEPLOYED; M4A live-connection gate remains OPEN.** Existing scoped authorization is retained. The #20 implementation adds OIDC sign-in, persistent browser sessions, repeat-safe principal provisioning, explicit workspace controls and local-draft/context protection; only the final current-source results above establish its local completion. Normal protected access fails closed without complete trusted configuration. **The single next task is separate selection and validation of the real provider/client registration, trusted HTTPS origin/callback and permitted production-database binding.** No live identity-provider operation, customer onboarding, deployed isolation or production migration is claimed.

**M4B is separate and NOT STARTED here:** full schema-5/historical-compatible physical saving, explicit legacy ownership review, additional additive persistence migrations, immutable revisions, autosave/idempotency/conflicts and scoped recovery. **M4C is separate and NOT STARTED here:** authorized PDF/CSV/print exports and remaining project list/duplicate/archive/restore workflows. The foundation's legacy rename/delete routes do not complete these features. Keep #4 PROPOSED and M5–M8 gates unchanged; object catalogs, kitchen/bath tools, trade overlays and overview views are not prerequisites.
...
Current state: M1–M3, the initial M3D foundation and #19 authorization foundation remain complete locally. The #20 OIDC/session/account implementation is complete locally: COMPLETE / PRODUCER_VERIFIED locally; NOT DEPLOYED; application/test c79e458a634131f9e25ed0e4b0bb7497fc07d7ae, checks/source match/canonical smoke as recorded in the current checkpoint above. **NOT DEPLOYED.** Publication commit IDs and GitHub convergence are recorded in issue #20. M4A stays incomplete because the real provider/client-registration, trusted HTTPS origin and permitted production-database binding have not been selected/verified. That separately authorized live-connection validation is the single next task. M4B full versioned physical persistence and M4C authorized exports are separate, NOT STARTED here. #9 remains open, #10 tracks release, #2 unreleased, #4 PROPOSED and CRM separate; later trade/M7 boundaries are unchanged.
