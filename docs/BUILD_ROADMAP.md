...
Version: 1.16 — M4A authorization foundation verified, 2026-09-09
Repository: `armentrout1/ModernFloorPlanner`; delivery branch: `main`.
Original audit baseline: 876968e78d7070775e7924f33a3164ba20905d42; current application/test commit: d4f6bb5471dca2d0347c94499226b73e17ac4093.
Status: **M4A authorization foundation COMPLETE / PRODUCER_VERIFIED locally; NOT DEPLOYED.** Initial M3D, M3B and M3C results are preserved. M4A as a whole is incomplete. **Single next task: real identity-provider/session integration and account/workspace UI with explicit account-transition and local-draft protections.** Broader #9 stays open, #10 remains release tracking, #2 unreleased, #4 PROPOSED and CRM separate.

Evidence and dated external research: [research and audit](RESEARCH_AND_AUDIT_2026-09-06.md).

## 0. Current M4A authorization-foundation checkpoint — 2026-09-09
...
**Single next implementation task: real identity-provider/session integration and account/workspace interface, including account-transition and local-draft protections.** Provider/issuer, trusted origin and permitted database binding must be explicitly selected and verified; no real sign-in is fabricated here. M4B follows with full versioned physical saving/revisions/conflicts; M4C adds authorized exports and remaining project management. Hosting, paid services, onboarding, new drawing features, material recipes and cross-product work remain excluded. This checkpoint supersedes historical next-task wording below without reopening completed M3 work.
...
At this preserved M3D checkpoint, M4A was next. Its authorization foundation is now complete above; real provider/session integration and account-facing safeguards remain next. Preserve schema 5 and historical source/snapshot compatibility in the persistence contract. Future object families, catalogs, detailed kitchen/bath tools, overview/ghost views, trade systems and M7 recipes are not automatic prerequisites. The user pilot speed/adoption target remains unmeasured. #10 tracks release, #2 remains unreleased and hosting #4 remains PROPOSED.
...
**M4A authorization foundation is complete above; its real provider/session and account/workspace slice is next.** No live identity or deployment/database binding is claimed. Current schema 5, full source lineage and v1–v5 capture compatibility must be explicitly covered when secure persistence/export interfaces are selected. Never save a partial legacy representation as though it preserved the current building document.

### M4 — Make standalone use safe and dependable

**M4A foundation COMPLETE locally in #19; M4A remains incomplete.** Actual server identity, workspace membership and resource-authorization enforcement, additive ownership schema, negative HTTP/browser tests and actual disposable-PostgreSQL repository checks are delivered. Normal protected access fails closed while no provider is configured. The next slice selects/integrates the real provider/session and account/workspace interface with explicit account-transition/local-draft protection and verified permitted configuration. No account onboarding or deployed isolation is claimed.

M4B remains full schema-5/historical-compatible physical saving, explicit legacy ownership review, additional additive persistence migrations, immutable revisions, autosave/idempotency/conflicts and scoped recovery. M4C remains authorized PDF/CSV/print exports and remaining project list/duplicate/archive/restore workflows. The foundation's legacy rename/delete routes do not complete these features. Keep #4 PROPOSED and M5–M8 gates unchanged; object catalogs, kitchen/bath tools, trade overlays and overview views are not prerequisites.
...
Current state: M1–M3 and the initial M3D foundation remain complete locally. The M4A authorization foundation in #19 is COMPLETE / PRODUCER_VERIFIED locally; NOT DEPLOYED. Implementation d4f6bb5471dca2d0347c94499226b73e17ac4093 passes final 535/535 unit/API, 180/180 browser and 15/15 separate PostgreSQL tests, install/check/build and working/staged whitespace checks. All 285 integrated source hashes match, and 8/8 separate normal-composition smoke checks pass at the fresh 5187 review. [Current results](MFP_M4A_AUTHORIZATION_RESULTS.md) record verification and limits. M4A as a whole remains incomplete: the single next task is real identity-provider/session integration and account/workspace UI with account-transition/local-draft protections. No provider, live database binding or deployment is verified. M4B full versioned saving and M4C authorized exports follow. #9 remains open, #10 tracks release, #2 is unreleased, #4 stays PROPOSED and CRM separate; later trade/M7 boundaries are unchanged.
