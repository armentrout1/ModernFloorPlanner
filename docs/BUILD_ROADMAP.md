...
Version: 1.15 — Initial M3D building-layout foundation verified, 2026-09-09
Repository: `armentrout1/ModernFloorPlanner`; delivery branch: `main`.
Original audit baseline: 876968e78d7070775e7924f33a3164ba20905d42; current application/test commit: 9d19deae6328e5b47b2754ff5f81d55847a64540.
Status: **Initial M3D foundation COMPLETE / PRODUCER_VERIFIED locally; NOT DEPLOYED.** Levels #16, linked straight stairs/landings/explicit surface openings #17, and minimal room-use/functional-zone/fixed-cabinet package #18 complete the agreed three-part foundation. M3B and M3C retain their local completion. **Next eligible bounded task: M4A — secure accounts and workspace authorization, NOT STARTED.** Its entry work verifies the intended deployment/database binding and records the actual identity-provider decision. Future object/catalog/trade/overview work is separately scoped, not an M4 prerequisite. Broader #9 remains open, #10 remains the release tracker, #2 unreleased, #4 PROPOSED and CRM separate.

Evidence and dated external research: [research and audit](RESEARCH_AND_AUDIT_2026-09-06.md).

## 0. Current M3D foundation checkpoint — 2026-09-09
...
**Single next eligible task: M4A — secure accounts and workspace authorization, NOT STARTED.** Verify the intended deployment/database binding and record the actual identity provider before account-facing implementation or onboarding. Preserve schema 5 and historical source/snapshot compatibility in the persistence contract. Future object families, catalogs, detailed kitchen/bath tools, overview/ghost views, trade systems and M7 recipes are not automatic prerequisites. The user pilot speed/adoption target remains unmeasured. #10 tracks release, #2 remains unreleased and hosting #4 remains PROPOSED.
...
**Next eligible bounded task: M4A — secure accounts and workspace authorization, NOT STARTED.** Its first work verifies the intended deployment/database binding and records the actual identity-provider decision. Current schema 5, full source lineage and v1–v5 capture compatibility must be explicitly covered when secure persistence/export interfaces are selected. Never save a partial legacy representation as though it preserved the current building document.

### M4 — Make standalone use safe and dependable

**Entry / next eligible bounded task: M4A — secure accounts and workspace authorization, NOT STARTED.** M1–M3 core workflow and the initial M3D foundation are implemented locally. First verify the intended deployment/database binding and record the actual identity-provider decision; resolve that provider/configuration gate before account-facing implementation or onboarding. Record how current schema 5 and historical source/snapshot versions are preserved by secure persistence. Future object catalogs, kitchen/bath tools, trade overlays and overview views are not prerequisites. M4A covers authentication, workspace/membership authorization and denied-access tests. M4B remains explicit legacy ownership review, additive storage migrations, revisions, autosave/idempotency/conflicts and local recovery. M4C remains project list, rename/duplicate/archive/restore; PDF/CSV and accessible print view from the same snapshot. This roadmap update does not activate M4A or hosting proposal #4.
...
Current state: M1, M2A/B/C, M3A, M3B and M3C remain complete locally. **The initial M3D foundation — levels #16, linked straight stairs/landings/surface openings #17 and minimal room-use/zones/fixed cabinetry #18 — is COMPLETE / PRODUCER_VERIFIED locally; NOT DEPLOYED.** Application/test `9d19deae6328e5b47b2754ff5f81d55847a64540` passes the actual final **516/516 unit and 174/174 browser checks**; canonical smoke: PASS — all 275 canonical source hashes match; working/staged Git whitespace checks pass; HTTP 200 and 1/1 browser smoke at [the fresh 5186 review](http://127.0.0.1:5186/physical-draft), 9.808 s command time (9.1 s Playwright), against `9d19deae6328e5b47b2754ff5f81d55847a64540`. [Current results](MFP_M3D_OBJECTS_ZONES_RESULTS.md) identify all checks and limits. **Next eligible bounded task: M4A secure accounts and workspace authorization, NOT STARTED**, beginning with deployment/database binding verification and the actual identity-provider decision. Broader #9 remains open for later requirements; future catalogs/trade/overview work is not an automatic M4 prerequisite. #10 remains the release tracker, #2 unreleased, #4 PROPOSED and CRM separate. Later trade and M7 material-recipe/purchasing boundaries remain unchanged.
