...
Version: 1.2 - M3B entry design and fresh integration baseline, 2026-09-08
...
Status: **M3 in progress; NOT DEPLOYED. M3A and assigned legacy-editor fixes are implemented; the fresh integration baseline is 234/234 unit checks and 95/96 browser checks. M3B entry design is documented but its gate is BLOCKED by one stale-coordinate browser regression. Single next implementation task: correct that regression and rerun the full isolated baseline under issue #10. M3B feature work and remaining M3C work have not started; M3D remains PROPOSED; M4-M8 remain unstarted.**
...
[Entry-gate design and exact evidence](MFP_M3B_ENTRY_GATE.md) / [MFP-M3B issue #10](https://github.com/armentrout1/ModernFloorPlanner/issues/10) incorporate [issue #9's ceiling-height and complete-takeoff addition](https://github.com/armentrout1/ModernFloorPlanner/issues/9#issuecomment-5586261905). This assignment is documentation and checks only, not implementation activation.

**Repository/publication:** verified existing main tracking origin/main. Checkpoint `6b8ef92d2ea4347918a95b7c5502f603d78820d8` changed this roadmap only and was the sole unpublished commit over `91402e8ced8b0121b12d1f53e54714d14fe5f173`; safe fetch found no divergence and the checkpoint was fast-forward pushed. Local/remote source baseline is 6b8ef92; application code remains 91402e8. This entry-gate publication adds documentation only. No competing local writer was observed. Open owner sketches, Quick Rooms draft, dev server/dependencies, stash and archive were preserved; no owner-tab interaction or reload.

**Fresh full integration baseline:** isolated tracked-source archive of 6b8ef92, Node 20.20.2 / npm 10.8.2. npm ci PASS (505 added/506 audited); npm test PASS **234/234**; npm run check PASS; npm run build PASS; exact `npx playwright test --reporter=line` **FAIL: 95 passed, 1 failed, 96 total**, one worker, zero retries, no replacement targeted run. git diff --check PASS. All 232 source files verified before/after, allowing only Git CRLF conversion. Full command details and sanitized evidence are linked above. npm reported 28 advisories (4 low, 10 moderate, 14 high); old browser-data/large-bundle warnings remain. No dependencies were changed. These are actual current results, not summed historical runs.

**Gate BLOCKED; single next task:** correct stale pre-selection screen coordinates in `tests/browser/canvas-view.spec.ts:182` (assertion 199). At narrow width, selecting a room wraps the controls and recenters the fit; the test then clicks a different room using the old bounds. Refresh target bounds/scale after selection and tool activation settle, preserving exact placement/no-other-room-mutation/resize assertions; do not weaken the test or redesign the UI. Then run the focused regression and a fresh full isolated baseline on the corrected source. No fix was made in this entry task. A diagnosed harness failure still prevents a complete-suite pass claim.

**Selected M3B design:** one selected physical-v2 draft shared by Quick Rooms, room inspector, derived drawing and quantities; explicit copy/adoption of one source without merging independent drafts. Both views edit existing physical ceilingHeight. A typed, versioned applicability contract distinguishes supported flat ceilings/uniform walls/crown paths from unknown or unsupported conditions and blocks only affected outputs. Versioned importer/recovery preserves newer groups/window heights/door appearance, IDs, raw input, measurement evidence and captured originals; physical documents never use lossy legacy save. Old adapters/snapshots retain their original semantics. Exact contracts and acceptance are in the linked entry record.

**First M3B feature slice after gate clearance:** shared draft/ceiling-height round trip, explicit source adoption and full temporary recovery, one selected room inspector, read-only physical drawing and shared floor/flat-ceiling/gross-wall results. Opening/work forms and writable geometry follow in bounded M3B slices. This slice is specified, not started. Quick Rooms and the current sketch remain separate today. Measured quantities are M3B; specified recipes/accessories/coverage/waste/purchasing are M7; construction/trade models remain later bounded assignments. No global full-materials claim from floor areas.

Deployment/real database/authentication/production smoke remain NOT VERIFIED; no hosting or database operation occurred. M4 still gates customer access. Keep all useful UI fixes, React/Vite and the existing engine. Historical evidence below describes its own checkpoints; this section controls current readiness.
...
Current state: M2A producer implementation is recorded under [issue #3](https://github.com/armentrout1/ModernFloorPlanner/issues/3), M2B under [issue #5](https://github.com/armentrout1/ModernFloorPlanner/issues/5), M2C shared engine/snapshot/parity evidence under [issue #6](https://github.com/armentrout1/ModernFloorPlanner/issues/6), and M3A Quick Rooms under [issue #7](https://github.com/armentrout1/ModernFloorPlanner/issues/7). All assigned slices are complete locally and NOT DEPLOYED. M1 issue #2 remains open and explicitly unreleased. Current M3B entry design is published under issue #10 and the linked entry record, including issue #9's latest ceiling-height/takeoff requirements. Fresh baseline is 234/234 unit and 95/96 browser tests; entry is BLOCKED. Single next implementation task: bounded stale-coordinate regression repair and a fresh complete baseline. The shared-draft/ceiling-height feature slice follows only after clearance; M3B features have not started. Section 0 controls current status; prior UI evidence is historical through application 91402e8. Issue #9 remains reconciled into A8, proposed M3D and M7 boundaries. M3D and detailed kitchen/bath/trade work remain unstarted.
